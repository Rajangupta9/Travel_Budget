...
const ExpenseTable = ({
  ...
  const formatDate = (dateString) => {
    ...
    if (expenseDate.toDateString() === today.toDateString()) {
      return `Today, ${expenseDate.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      })}`;
    }
    ...
    if (expenseDate.toDateString() === yesterday.toDateString()) {
      return `Yesterday, ${expenseDate.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      })}`;
    }
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-10">
        <div className="bg-white rounded-lg shadow-xl max-w-md w-full m-4 p-6">
          ...
          <p className="text-gray-600 mb-4">
            ...
            <br />
            <span className="font-medium">{confirmDelete.description}</span> (
            {currency}
            {parseFloat(confirmDelete.amount).toFixed(2)})
          </p>
            ...
            <div className="mt-3">
              ...
              <div className="flex flex-wrap gap-1">
                {expense.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-2 py-1 bg-gray-200 text-xs rounded-full text-gray-700"
                  >
                    {tag}
                  </span>
                ))}
      ...
      {filteredExpenses.length > 0 && (
        <div className="px-6 py-2 bg-gray-50 text-sm border-b border-gray-200 flex justify-between">
          <span>
            ...
            expenses
            {searchTerm && (
              <span> (filtered from {recentExpenses.length})</span>
            )}
          </span>
          <span>
            Total:{" "}
                      ...
                      <div className="flex justify-end space-x-2">
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setExpandedRow(
                              expandedRow === expense.id ? null : expense.id
                            );
                          }}
                          title="View details"
                        >
                    ...
                    {searchTerm ? (
                      ...
                      <>
                        ...
                        <button
                          ...
                          onClick={() =>
                            activeTrip && setShowAddExpenseModal(true)
                          }
                          disabled={!activeTrip}
                        >
